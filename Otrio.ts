...
document.addEventListener("DOMContentLoaded", () => {
    ...
    for (let i = 0; i < coll.length; i++)
        coll[i].addEventListener("click", (e) => {
            ...
            var content = <HTMLDivElement>coll[i].nextElementSibling;
            content.style.maxHeight = content.style.maxHeight ? null : "fit-content";
        });
    var colors_base = <HTMLDivElement>document.getElementById("colors");
    (<HTMLInputElement>document.getElementById("bkgrd_color")).value = background_color.substring(0, 7);
    (<HTMLInputElement>document.getElementById("empty_color")).value = empty_color.substring(0, 7);
    function create_color(idx: number) {
        var n = document.createElement("div");
        var id = <HTMLLabelElement>document.createElement("label");
        function set_color_props(sub_catagory: string): HTMLInputElement {
    ...
    let e_alp_elem = <HTMLInputElement>document.getElementById("blank-alpha");
    let c_alp_elem = <HTMLInputElement>document.getElementById("corpse-alpha");
    ...
    e_alp_elem.addEventListener("change", () => {
        ...
        exchange_colors(<any>document.getElementById("empty_color"), null, 0);
    });
...
function exchange_colors(elem: HTMLInputElement, idx, is_corpse) {
...
class Player {
    ...
    makeMove(circle_info: circle_info) {
...
class otrio {
    ...
    inRange: Path2D;
    paths: Path2D[];
    constructor(x: number, y: number, c: string = null) {
        this.x = x;
        this.y = y;
    ...
    getCircle(e: MouseEvent) {
        for (let i = 0; i < this.circles.length; i++)
            if (ctx.isPointInStroke(this.paths[i], e.offsetX, e.offsetY)) return i;
        return null;
    }
}
class circle_info {
...
function c_swap(idx: any, old_c: string, new_c: string, is_corpse: any) {
...
function draw_game_board() {
    ...
    (<any>ctx).roundRect(line_width / 2, spacing + line_width / 2, game_board_width, game_board_height, circle_gap);
    (<any>ctx).roundRect(spacing + line_width / 2, line_width / 2, game_board_height, game_board_width, circle_gap);
    ctx.fill();
    ctx.closePath();
    ...
    ctx.beginPath();
    (<any>ctx).roundRect(spacing + line_width, spacing + line_width, game_board_height, game_board_height, circle_gap);
    ctx.stroke();
    ctx.closePath();
...
function getSelectedCircle(e: MouseEvent): circle_info {
...
function checkWin(circle_info: circle_info) {
...
function getSettings() {
    input_players = input_players ? input_players : <HTMLInputElement>document.getElementById("player-count");
    input_size = input_size ? input_size : <HTMLInputElement>document.getElementById("game-width");
    check_box = check_box ? check_box : <HTMLInputElement>document.getElementById("checkbox-corpse");
    input_width = input_width ? input_width : <HTMLInputElement>document.getElementById("game-size");
    if (!canvas) {
...
function is_cats() {
    ...
    for (let i = 0; i < num_players; i++)
        player_matches &&= players[i].pieces.every((c) => c.get_match(null) == (1 << width) - 1);
